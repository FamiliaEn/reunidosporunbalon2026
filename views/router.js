...
//ruta para enviar los datos en formato json
router.get('//Ipais/' + data +1', (req, res)=>{     
...
//    console.log('Debug quiniela 2 ',req.session.Alias);
    ...
    union all \
    SELECT Q.Id,P.Id Id_P,Q.clave ClaveL,P.Local,null,null,Q.equipo,V.Clave ClaveV,P.Fecha,P.Horario,P.Estadio,Q.Estatus Estatus FROM partidos as P, paises as V, campeon as Q WHERE P.Id > 109 and Q.equipo = V.nombre and P.Id = Q.Id_partido and Q.Id_participante=? \
...
router.get('/quinielas_data', (req, res)=>{     
    ...
    const Id_p = req.session.Id_participante;
    console.log('Folder 3',global.globalFolder);
...
//    console.log('Debug quiniela A ',req.session.Alias);
    ...
    and Q.Id_participante=R.Id_participante and Q.Id_participante=F.Id_participante and P.Estatus=1 \
    union all \
    ...
    WHERE P.Id > 109 and Q.equipo = V.nombre and P.Id = Q.Id_partido and Q.Id_participante=R.Id_participante and Q.Id_participante=F.Id_participante \
...
//    console.log('Debug quiniela A ',req.session.Alias);
    ...
    FROM partidos as P, paises as L, paises as V, quiniela as Q, participantes R   \
    WHERE P.Visitante = V.nombre and P.Local = L.nombre and P.Id = Q.Id_partido \
    and Q.Id_participante=R.Id_participante and Q.Id_participante=F.Id_participante and R.Estatus = 1 \
    union all \
    SELECT Q.Id,concat_ws(P.Descripcion, P.Id,  P.Id)  Id_P ,R.Alias,Q.clave ClaveL,P.Local,null,null,Q.equipo,V.Clave ClaveV \
    FROM partidos as P, paises as V, campeon as Q, participantes R \
    WHERE P.Id > 109 and Q.equipo = V.nombre and P.Id = Q.Id_partido and Q.Id_participante=R.Id_participante and Q.Id_participante=F.Id_participante and R.Estatus = 1 \
    order by 1,2',[global.globalFolder,global.globalFolder],(error, results)=>{
        if(error){
            throw error;
        } else {                                                   
...
router.get('/participantes_datF1', (req, res)=>{ 
    ...
    pool.query('SELECT L.Lugar Lugar, P.Alias Alias, P.Puntos Puntos FROM lugar L, participantes P where L.Id_participante = P.Id_participante and p.Estatus = 1 order by P.Fecha',[global.globalFolder] ,(error, results)=>{
         if(error){
            console.log(error);
            console.error(err);
            return;
