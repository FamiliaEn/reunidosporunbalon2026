...
router.get('/calendario_data', (req, res)=>{     
    conexion.query('SELECT P.Id,P.Local,L.clave ClaveL,V.Clave ClaveV,P.Visitante,date_format(P.Fecha, "%d-%m-%Y") as Fecha,P.Horario,P.Estadio,P.Estatus FROM partidos as P, paises as L, paises as V \
...
router.get('/participantes_correo_dat', (req, res)=>{     
    conexion.query('SELECT Id_participante, Nombre, Alias, Contacto, Correo, Estatus, Fecha FROM participantes ORDER BY Nombre, Alias',(error, results)=>{
        if(error){
            throw error;
        } else {                                                   
            datap = JSON.stringify(results);
